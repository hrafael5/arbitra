...
module.exports = (ctrl) => {
  ...
  ctrl.storage.streamTick = (stream, streamID) => {
    ...
    if (streamID == 'allMarketTickers') {
      ...
      if (pairToTrade != 'none') {
        ctrl.logger.info("Oportunidade de arbitragem encontrada! Iniciando negociação: " + JSON.stringify(pairToTrade));
        // Aqui está a chamada para executar a negociação real
        // Observe que mudamos de executeTrade para initiateTrade, conforme definido no TradingCore
        this.tradingCore.initiateTrade(pairToTrade)
          .then(() => {
            ctrl.logger.info("Negociação iniciada com sucesso!");
          })
          .catch(error => {
            ctrl.logger.error("Erro ao iniciar negociação:", error);
          });
      }
  ...
  this.tradingCore = TradingCore(ctrl.options, ctrl.currencyCore, ctrl); // Pass full ctrl.options object
